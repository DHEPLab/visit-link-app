...
export default function DatePicker({ value, onChange, ...props }) {
  ...
  function onChangeTime({ nativeEvent }) {
    ...
    if (nativeEvent.timestamp) {
      ...
      onChange(date.format(i18n.language === 'zh' ? 'YYYY年MM月DD日' : 'YYYY-MM-DD'));
    }
  }

  function formatDate(date) {
    return moment(date).format(i18n.language === 'zh' ? 'YYYY年MM月DD日' : 'YYYY-MM-DD');
  }

  return (
      <Container>
        <TouchableOpacity onPress={onPressTime} activeOpacity={0.8}>
          {value ? <Value>{formatDate(value)}</Value> : <Placeholder>{t('selectDate')}</Placeholder>}
        </TouchableOpacity>
        {visible && (
            <DateTimePicker
                {...props}
