...
export default function Babies({navigation}) {
    ...
    function backupBabyAndCaregivers() {
        http.get('/api/babies', {page: 0, size: 1000})
            .then((data) => {
            ...
            {isConnected ?
                <BackupLine>
                    <TouchableOpacity activeOpacity={0.8} onPress={backupBabyAndCaregivers}>
                        <PromptWord>{t('backupPrompt')}<Link>{t('backupLink')}</Link></PromptWord>
                    </TouchableOpacity>
                </BackupLine> :
                <BackupLine>
                    <PromptWord><AntDesign name="infocirlceo" size={px2dp(8)} color="#ACA9A9"/>{t('offlineMode')}
                    </PromptWord>
                </BackupLine>
            }
            {contents.length > 0 && (
                <ListHeader>
                    <TitleContainer>
                        ...
                        {isConnected && <TouchableOpacity activeOpacity={0.8} onPress={() => {
                            ...
                            <SortLine>{t('sortBy')}
                                <SortField sortType={sortName}
                                           onPress={searchBySortName}> {t('name')} {sortName === 'asc' ? '↑' : sortName === 'desc' ? '↓' : '⇅'}</SortField>&nbsp;&nbsp;&nbsp;&nbsp;
                                <SortField sortType={sortDate}
                                           onPress={searchBySortDate}> {t('creationTime')} {sortDate === 'asc' ? '↑' : sortDate === 'desc' ? '↓' : '⇅'}</SortField>
                            </SortLine>
                        </TouchableOpacity>}
                        <InfoLine>
                    ...
                    <Button onPress={() => navigate('CreateBabyStep1')} title={t('addBaby')}/>
                </ListHeader>
            )}
                        ...
                        <NoDataContainer>
                            {name ? (
                                ...
                                <>
                                    ...
                                    <Button title={t('addBaby')} onPress={() => navigate('CreateBabyStep1')}/>
                                </>
                            )}
                        </NoDataContainer>
                    )
